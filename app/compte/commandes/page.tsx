...
//   const orders = getAllOrders().filter((order: any) => order.userId === user.id);
...
// 🟦 Define the type for all possible order statuses
type OrderStatus =
  | "pending"
  | "confirmed"
  | "processing"
...
// 🟩 Define related TypeScript interfaces for your data
interface Product {
  name: string;
  price: number;
}
...
// 🟨 Typed objects (no implicit any errors)
const statusIcons: Record<OrderStatus, React.FC<LucideProps>> = {
  pending: Clock,
  confirmed: CheckCircle,
  processing: Package,
  shipped: Truck,
...
export default function OrdersPage() {
  ...
  // ✅ Explicitly cast the result to UserOrder[]
  const orders = (getAllOrders() as unknown as UserOrder[]).filter(
