...
const CartModal = ({ onClose }: CartModalProps) => {
    ...
    const handleCheckout = async () => {
        ...
        try {
            const orderData = {
                items: items.map(item => ({
                    ...
                    unit_price: typeof item.product.price === 'string'
                        ...
                        : item.product.price,
                    total_price: (typeof item.product.price === 'string'
                        ? parseFloat(item.product.price)
            ...
            setError(err instanceof Error ? err.message : "Échec de la création de la commande");
        } finally {
                        ...
                        {!isCheckingOut ? (
                            <>
                                <div className="space-y-4 mb-4 max-h-96 overflow-y-auto">
                                    {items.map((item) => (
                                        <div key={`${item.product.id}-${item.quantity}`} className="flex items-start border-b pb-4">
                                ...
                                <div className="border-t pt-4">
                                    <div className="flex justify-between mb-4">
                                        <span>Total ({totalItems} {totalItems > 1 ? 'articles' : 'article'})</span>
